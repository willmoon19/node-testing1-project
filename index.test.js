...
describe('[Exercise 2] trimPropertiesMutation', () => {
  test('[3] returns an object with the properties trimmed', () => {
    const input = { name: "  will  " }
    const expected = { name: "will" }
  ...
  test('[4] the object returned is the exact same one we passed in', () => {
    const input = { name: "  will  " }
    utils.trimPropertiesMutation(input)
    expect(input).toMatchObject({ name: "will" })
  })
})
...
describe('[Exercise 3] findLargestInteger', () => {
  test('[5] returns the largest number in an array of objects { integer: 2 }', () => {
    ...
    const expected = utils.findLargestInteger(numbers)
...
describe('[Exercise 7] isEvenNumberAsync', () => {
  ...
  test('[21] rejects an error with the message "number must be a number" if passed a non-number type', async () => {
    try {
      ...
      expect(err.message).toMatch("number must be a number")
    }
  })
